...
public class LoaderObjectPropertyEquivalentToClassTest {

    final File ontology = new File("src/test/resources/LoaderObjectPropertyEquivalentToClass/ontology.owl");
    ...
    void shouldReturnClassIndividuals() throws OWLOntologyCreationException {
        ...
        List<SimpleStatement> results = new LoaderObjectPropertyEquivalentClasses(mockedDatabase, manager, reasoner)
        ...
        assertEquals(i, results.get(0).getNamedValues().get(CqlIdentifier.fromCql("cls")));
        assertEquals(p, results.get(0).getNamedValues().get(CqlIdentifier.fromCql("equiv")));
    }
