...
public class ReasonerEquivalentToSome extends ReasonerManager {

    private final static Logger LOGGER = Logger.getLogger(ReasonerEquivalentToSome.class);

    public ReasonerEquivalentToSome(Database connection, SparkConf conf, JedisPool pool) {
        super(connection, conf, pool);
    }

    public ReasonerEquivalentToSome(Database connection, JavaSparkContext sc, JedisPool pool) {
    ...
    public List<Tuple2<String, String>> inference() {
        ...
        JavaPairRDD<String, IsEquivalentToSome.Row> isEquivalentToSomeRDD = javaFunctions(spark)
                ...
                .keyBy((Function<IsEquivalentToSome.Row, String>) IsEquivalentToSome.Row::getRange);

        if (LOGGER.isDebugEnabled())
            isEquivalentToSomeRDD.foreach(data -> {
                LOGGER.debug("isequivalenttosome range=" + data._1() + " row=" + data._2());
            });
        ...
        JavaPairRDD<String, Tuple2<IsEquivalentToSome.Row, String>> firstJoinRDD = isEquivalentToSomeRDD
                ...
                .mapToPair(tuple -> {
                    String individual = tuple._2()._2();

                    IsEquivalentToSome.Row isEquivalentToSomeRow = tuple._2()._1();
        ...
        JavaPairRDD<String, String> finalRDD = secondJoinRDD
                // Filters out rows where individual == range
                .filter(tuple -> {
                    ...
                    return individual.equals(propIndividualsRow.getRange());
                })
                .mapToPair(tuple -> {
                    Tuple2<IsEquivalentToSome.Row, PropIndividuals.Row> secondOperand = tuple._2();
                    IsEquivalentToSome.Row isEquivalentToSomeRow = secondOperand._1();
                    PropIndividuals.Row propIndividualsRow = secondOperand._2();
                    return new Tuple2<>(isEquivalentToSomeRow.getCls(), propIndividualsRow.getDomain());
                });
